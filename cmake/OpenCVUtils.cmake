...
macro(ocv_cmake_hook hook_name)
  ...
  foreach(__hook ${${__var_name}})
	...
	# https://cmake.org/cmake/help/v3.13/command/if.html#command:if
	#это проверка является ли строка __hook именем команда которую можно запустить:
    if(COMMAND "${__hook}")
      message(FATAL_ERROR "Indirect calling of CMake commands is not supported yet")
    else()
	#подключение и ЗАПУСК Cmake файла:
      include("${__hook}")
    endif()
	#Итого выше либо убедились что это имя CMake процедуры и нажаловались, либо подключили и запустили CMake файл
  endforeach()
endmacro()
...
#Подключаем все CMake файлы из каталога:
...
# Если переменная определена:
  if(DEFINED ${var_name})
  #https://cmake.org/cmake/help/v3.13/command/file.html#command:file
  #То мы создаём файл с именем согласно имени переменной и копируем в него содержимое переменной
    file(WRITE "${CMAKE_BINARY_DIR}/CMakeCommand-${var_name}.cmake" ${${var_name}})
  # ЗАпускаем этот файл:	
    include("${CMAKE_BINARY_DIR}/CMakeCommand-${var_name}.cmake")
  endif()
  # Чистим переменную чтобы память не ела:
...
#Перенос системных переменных из environment в локальные:
